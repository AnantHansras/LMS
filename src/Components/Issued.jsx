...
const Issued = ({ books }) => {
  ...
  const BookCard = ({ book, index }) => (
    <motion.div
      className="relative backdrop-blur-2xl rounded-xl p-5 shadow-lg hover:shadow-2xl w-[285px] h-[420px] mx-auto flex flex-col cursor-pointer transition-transform transform hover:scale-105"
      onClick={() => openModal(book)}
      whileHover={{ scale: 1.03 }}
      whileTap={{ scale: 0.98 }}
      style={{
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="relative w-full max-w-md">
              ...
              <input
                type="text"
                placeholder="Search by title, author or genre..."
                className="pl-10 pr-4 py-3 w-full rounded-full focus:outline-none transition-all duration-300"
                value={searchQuery}
                onChange={(e) => {
                  const value = e.target.value;
                  setSearchQuery(value);
